...
export function DashboardPage(): JSX.Element {
  const statsQuery = useQuery({ queryKey: ["task-stats"], queryFn: statsApi.tasksSummary });
  const tasksQuery = useQuery({ queryKey: ["tasks", "dashboard"], queryFn: () => tasksApi.list({ page: 1, page_size: 6 }) });
  const executionsQuery = useQuery({ queryKey: ["executions", "dashboard"], queryFn: () => executionsApi.list({ page: 1, page_size: 8 }) });
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {statsQuery.isLoading ? (
          ...
          <>
            ...
            <MetricCard icon={<CheckCircle2 className="h-4 w-4 text-emerald-500" />} label="成功执行" value={stats?.success_executions ?? 0} hint={`成功率 ${stats?.success_rate ?? 0}%`} />
            <MetricCard icon={<TimerOff className="h-4 w-4 text-destructive" />} label="失败执行" value={stats?.failed_executions ?? 0} hint={`${stats?.total_executions ?? 0} 次总执行`} />
            <MetricCard icon={<PlayCircle className="h-4 w-4 text-amber-500" />} label="运行中" value={stats?.running_executions ?? 0} hint="实时任务状态" />
          </>
        )}
      </div>
              ...
              {(tasksQuery.data?.items ?? []).map((task) => (
                <div key={task.id} className="rounded-lg border p-3">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <Badge variant={task.is_active ? "default" : "secondary"}>{task.is_active ? "启用" : "停用"}</Badge>
                  </div>
              ...
              <table className="data-table">
                ...
                <tbody>
                  ...
                  {(executionsQuery.data?.items ?? []).map((item) => (
                    <tr key={item.id}>
                      <td className="max-w-[14rem] truncate">{item.task?.name ?? `#${item.task_id}`}</td>
                      <td>
